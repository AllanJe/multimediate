...
#'@param indictrans data.frame indicating names of non-linear variables that have to be bactransform using Box-Cox likelihood medthod.
#'@param mediatort names of all mediators (non-transformed) in the order of lmodel.m
#'
...
#' @export
#'
#' @importFrom car bcnPowerInverse
#' @importFrom MASS mvrnorm
#' @importFrom mvtnorm rmvnorm


# indictrans=data.frame(variables=c("med1","med2","med4","outcome"),
#            funct=c("bc","bc","bc","bcn"),
#            lambda=c(0.38,0.41,0.19,0.5),
#            gamma=c(NA,NA,NA,0.1))

#indictrans
#   variables funct lambda gamma
# 1      med1    bc   0.38    NA
# 2      med2    bc   0.41    NA
# 3   outcome   bcn   0.50   0.1

BCmultimediate=function(lmodel.m,correlated=FALSE,model.y,treat,treat.value=1,control.value=0,J=1000,conf.level=0.95,indictrans=NULL,mediatort=NULL){
  ...
  if (!is.null(mediatort)){
    namemediator=mediatort
  }
  else {
    namemediator=mediator
  }
  ...
  for (e in 1:4) {
    ...
    bPr0<-bPr1<- matrix(NA, nrow = N, ncol = J)
    if (NM!=1 & e<=2) {
      Pr0.NM<-Pr1.NM <-ORPr0.NM<-ORPr1.NM <- array(NA,dim=c(N,J,NM))
      bPr0.NM<-bPr1.NM <- array(NA,dim=c(N,J,NM))
    }
    ...
    for (j in 1:J) {
      ...
      for (nm in 1:NM){
        if(mediator[nm] %in% indictrans$variables){
          nmtr=which( indictrans$variables %in% mediator[nm] )
          if (indictrans$funct[nmtr]=="bc"){
            PredictMt[,nm]=bcPowerInverse(PredictMt[,nm],indictrans$lambda[nmtr])
            PredictMc[,nm]=bcPowerInverse(PredictMc[,nm],indictrans$lambda[nmtr])
          }
          else if (indictrans$funct[nmtr]=="bcn"){
            PredictMt[,nm]=bcnPowerInverse(PredictMt[,nm],indictrans$lambda[nmtr],indictrans$gamma[nmtr])
            PredictMc[,nm]=bcnPowerInverse(PredictMc[,nm],indictrans$lambda[nmtr],indictrans$gamma[nmtr])
          }

        }
      }
      pred.data.t[, namemediator] <- PredictMt
      pred.data.c[, namemediator] <- PredictMc
      ...
      for (nm in 1:NM){
        if(inherits(lmodel.m[[nm]],"polr")){
          pred.data.c[, namemediator[nm]]=as.factor(pred.data.c[, namemediator[nm]])
          levels(pred.data.c[, namemediator[nm]])=lmodel.m[[nm]]$lev
          pred.data.t[, namemediator[nm]]=as.factor(pred.data.t[, namemediator[nm]])
          levels(pred.data.t[, namemediator[nm]])=lmodel.m[[nm]]$lev
        }

        if(inherits(lmodel.m[[nm]],"lm") & !inherits(lmodel.m[[nm]],"glm")){
          pred.data.c[, namemediator[nm]]=as.numeric(pred.data.c[, namemediator[nm]])
          pred.data.t[, namemediator[nm]]=as.numeric(pred.data.t[, namemediator[nm]])
        }

        if(inherits(lmodel.m[[nm]],"glm")){
          pred.data.c[, namemediator[nm]]=as.factor(pred.data.c[, namemediator[nm]])
          levels(pred.data.c[, namemediator[nm]])=levels(lmodel.m[[nm]]$model[,1])
          pred.data.t[, namemediator[nm]]=as.factor(pred.data.t[, namemediator[nm]])
          levels(pred.data.t[, namemediator[nm]])=levels(lmodel.m[[nm]]$model[,1])
        }
      }
      ...
      if (!inherits(model.y,"polr")){
        bPr1[,j] <- t(as.matrix(YModel[j, ])) %*% t(ymat.t) + Yerror
        bPr0[,j] <- t(as.matrix(YModel[j, ])) %*% t(ymat.c) + Yerror
      ...
      if("outcome" %in% indictrans$variables){
        outr=which(indictrans$variables %in% "outcome")
        if (indictrans$funct[outr]=="bc"){
          Pr1[,j]=bcPowerInverse(Pr1[,j],indictrans$lambda[outr])
          Pr0[,j]=bcPowerInverse(Pr0[,j],indictrans$lambda[outr])
        }
        else if (indictrans$funct[outr]=="bcn"){
          Pr1[,j]=bcnPowerInverse(Pr1[,j],indictrans$lambda[outr],indictrans$gamma[outr])
          Pr0[,j]=bcnPowerInverse(Pr0[,j],indictrans$lambda[outr],indictrans$gamma[outr])
        }
      }
        ...
        for (nm in 1:NM){
          ...
          if (tt[2]==1){
            ...
            if(mediator[nm] %in% indictrans$variables){
              ...
              if (indictrans$funct[nmtr]=="bc"){
                PredictMt=bcPowerInverse(PredictMt,indictrans$lambda[nmtr])
                PredictMc=bcPowerInverse(PredictMc,indictrans$lambda[nmtr])
              }
              else if (indictrans$funct[nmtr]=="bcn"){
                PredictMt=bcnPowerInverse(PredictMt,indictrans$lambda[nmtr],indictrans$gamma[nmtr])
                PredictMc=bcnPowerInverse(PredictMc,indictrans$lambda[nmtr],indictrans$gamma[nmtr])
              }

            }

          if(sum(mediator[-nm] %in% indictrans$variables)>0){
            mnmtr=which( indictrans$variables %in% mediator[-nm] )
            wnmtr=which( mediator[-nm]  %in% indictrans$variables  )
            for(wm in 1:length(mnmtr)){
              if (indictrans$funct[mnmtr[wm]]=="bc"){
                PredictWt[,wnmtr[wm]]=bcPowerInverse(PredictWt[,wnmtr[wm]],indictrans$lambda[mnmtr[wm]])
                PredictWc[,wnmtr[wm]]=bcPowerInverse(PredictWc[,wnmtr[wm]],indictrans$lambda[mnmtr[wm]])
              }
              else if (indictrans$funct[mnmtr[wm]]=="bcn"){
                PredictWt[,wnmtr[wm]]=bcnPowerInverse(PredictWt[,wnmtr[wm]],indictrans$lambda[mnmtr[wm]],indictrans$gamma[mnmtr[wm]])
                PredictWc[,wnmtr[wm]]=bcnPowerInverse(PredictWc[,wnmtr[wm]],indictrans$lambda[mnmtr[wm]],indictrans$gamma[mnmtr[wm]])
              }
            }

          }

          pred.data.t[, namemediator[nm]] <- PredictMt
          pred.data.t[, namemediator[-nm]] <- PredictWt
          pred.data.c[, namemediator[nm]] <- PredictMc
          pred.data.c[, namemediator[-nm]] <- PredictWc
          for (nmt in 1:NM){
            if(inherits(lmodel.m[[nmt]],"polr")){

              pred.data.c[, namemediator[nmt]]=as.factor(pred.data.c[, namemediator[nmt]])
              levels(pred.data.c[, namemediator[nmt]])=lmodel.m[[nmt]]$lev
              pred.data.t[, namemediator[nmt]]=as.factor(pred.data.t[, namemediator[nmt]])
              levels(pred.data.t[, namemediator[nmt]])=lmodel.m[[nmt]]$lev
            }

            if(inherits(lmodel.m[[nmt]],"lm") & !inherits(lmodel.m[[nmt]],"glm")){
              pred.data.c[, namemediator[nmt]]=as.numeric(pred.data.c[, namemediator[nmt]])
              pred.data.t[, namemediator[nmt]]=as.numeric(pred.data.t[, namemediator[nmt]])
            }

            if(inherits(lmodel.m[[nmt]],"glm")){
              pred.data.c[, namemediator[nmt]]=as.factor(pred.data.c[, namemediator[nmt]])
              levels(pred.data.c[, namemediator[nmt]])=levels(lmodel.m[[nmt]]$model[,1])
              pred.data.t[, namemediator[nmt]]=as.factor(pred.data.t[, namemediator[nmt]])
              levels(pred.data.t[, namemediator[nmt]])=levels(lmodel.m[[nmt]]$model[,1])
            }
          }
          ...
          if(!inherits(model.y,"polr")){
            bPr1.NM[,j,nm] <- t(as.matrix(YModel[j, ])) %*% t(ymat.t)+ Yerror
            bPr0.NM[,j,nm] <- t(as.matrix(YModel[j, ])) %*% t(ymat.c)+ Yerror
            ...
            if("outcome" %in% indictrans$variables){
              outr=which(indictrans$variables %in% "outcome")
              if (indictrans$funct[outr]=="bc"){
                Pr1.NM[,j,nm]=bcPowerInverse(Pr1.NM[,j,nm],indictrans$lambda[outr])
                Pr0.NM[,j,nm]=bcPowerInverse(Pr0.NM[,j,nm],indictrans$lambda[outr])
              }
              else if (indictrans$funct[outr]=="bcn"){
                Pr1.NM[,j,nm]=bcnPowerInverse(Pr1.NM[,j,nm],indictrans$lambda[outr],indictrans$gamma[outr])
                Pr0.NM[,j,nm]=bcnPowerInverse(Pr0.NM[,j,nm],indictrans$lambda[outr],indictrans$gamma[outr])
              }
            }
  ...
  if (NM==1){
    out <- list(d0 = d0, d1 = d1, d0.ci = d0.ci, d1.ci = d1.ci, d0.p = d0.p, d1.p = d1.p, d0.sims = delta.0,d1.sims = delta.1,
                ...
                treat = treat, mediator = namemediator,
                conf.level = conf.level,
                model.y = model.y, model.m = lmodel.m,
                control.value = control.value, treat.value = treat.value,
                nobs = N, sims = J,cov=cov,cor=cor,PredictM0=PredictM0,PredictM1=PredictM1,Pr1=Pr1,Pr0=Pr0,bPr1=bPr1,bPr0=bPr0,PredictMt=PredictMt,PredictMc=PredictMc,PredictWt=PredictWt,PredictWc=PredictWc,Pr1.NM=Pr1.NM,Pr0.NM=Pr0.NM,bPr1.NM=bPr1.NM,bPr0.NM=bPr0.NM)
    if (!is.null(model.y$family)){
      if (model.y$family$link=="logit"){
        out <- list(d0 = d0, d1 = d1, d0.ci = d0.ci, d1.ci = d1.ci, d0.p = d0.p, d1.p = d1.p, d0.sims = delta.0,d1.sims = delta.1,
                    z0 = z0, z1 = z1, z0.ci = z0.ci,z1.ci = z1.ci, z0.p = z0.p, z1.p = z1.p, z0.sims = zeta.0, z1.sims = zeta.1,

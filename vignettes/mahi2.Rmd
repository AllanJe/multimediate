...
```{r}
set.seed(42)
nbrsamples <- 100 # number of datasets to simulate
...
#for (s in 1:nbrsamples) {
  ...
  #Y <- matrix(rbinom(n, 1, 1/(1 + exp(-Y))), nrow = n)
   ...
   simdatasets <- cbind(X, M, Y)#, C)
#   simdatasets[[s]] <- cbind(X, M, Y)#, C)
# }

   simdatasets <- as.data.frame(simdatasets)
colnames(simdatasets)[1] <- "X" # renaming the exposure as X
colnames(simdatasets)[p+2] <- "Y" # renaming the outcome as Y
...
Before using the function mahi2 we have to choose a value for lambda using lambdaChoice function. We have to set a max for lambda with lambdamax. The returned value is less than lambdamax and allows you to select a mediator number between selectedMin and selectedMax. The value of selectedMax must be less than or equal to the number of individuals n. If the value of lambdamax is not hifh enough you will have this message. 

```{r}
n=dim(simdatasets)[1] #sample number
P=1 #number of treatment
treatment=matrix(simdatasets$X,n,P)
mediators=as.matrix(simdatasets[,paste0("M", 1:p)])
outcome=simdatasets$Y


  lambda <- lambdaChoice(treatment,mediators,outcome,lambdamax=10,selectedMin=n/2,selectedMax=n,L0=.1,eta=2,tau=1/P,epsilon=.001)

```

You have to choose a greater value, for example here 300.
```{r}
lambda <- lambdaChoice(treatment,mediators,outcome,lambdamax=300,selectedMin=n/2,selectedMax=n,L0=.1,eta=2,tau=1/P,epsilon=.001)
lambda
```

Now you can use the mahi2 function for the selection
```{r}
  mahifit <- mahi2(simdatasets,name.exposure="X",name.outcome="Y",name.mediators=paste0("M", 1:p),lambda=lambda,Nboot=30,L0=.1,eta=2,tau=1/P,epsilon=.001,Kmax=NULL,p.adjust.method="bonferroni",pvalseuil=0.05)
```

The function mahi2 returns a list containing several results such as the mediators selected in the first step. To see the mediators selected after the two steps you can use these lines of codes.
```{r}
